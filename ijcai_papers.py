...
def parse_arguments():
    ...
    parser.add_argument('--year', type=str, default='2018',
                        help='YEAR')
    parser.add_argument('--save_path', type=str, metavar='PATH', default='C:\\研究项目\\IJCAIPapers',
...
def getIJCAIPapers(year, save_path, minnum, maxnum):
    ...
    for i in range(minnum, maxnum):
        ...
        file_path = '{}/{:04d}_{}.pdf'.format(save_path, i, TITLE_DICT[i])
...
if __name__ == '__main__':
    ...
    year = args.year
    save_path = '{}/{}'.format(args.save_path, year)
    ...
    papers = bs(html, 'html.parser').find_all(class_='paper_wrapper')
    print('Paper list extracted! Total number: {}'.format(len(papers)))
    for i, p in enumerate(papers):
        TITLE_DICT[i + 1] = re.sub('/|\\\|:|\*|\?|"|<|>|\||\\n', '', p.find(class_='title').text)

    threads = []
    cnt = len(papers) // thread_num + 1
